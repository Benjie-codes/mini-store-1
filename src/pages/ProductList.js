...
const ProductList = () => {
    ...
    return (
        <Container>
            <Group>
                <section className="py-12 text-center overflow-hidden mb-3">
                        ...
                        <div className="grid md:grid-cols-3 grid-cols-2 gap-10 p-6">
                            {products.map(product => (
                                <Card key={product.id} className='border p-4'>
                                    ...
                                    <Group className=''>
                                        <Text className='font-body text-l text-center md:text-base'>{product.name}</Text>
                                        <Badge className='font-bold text-l md:text-xl text-black'>
                                            ₦ {product?.current_price[0]?.["NGN"]?.[0]}
                                        </Badge>
                                    </Group>
                                    <div className='flex justify-center'>
                                        <button onClick={() => AddToCartAndNotify(product)} className="mt-2 w-full bg-black text-white px-4 py-2  hover:bg-white hover:text-black transition duration-500 ease-in-out hover:border-2 hover:border-black ">Add To Cart</button>
                                    </div>
            ...
            <Group className='flex justify-center gap-2 mb-5'>
                <Button onClick={() => setPage(page > 1 ? page - 1 : 1)} className='bg-black flex justify-center text-white px-4 py-2 '>Prev</Button>
                <Button onClick={() => setPage(page + 1)} className='bg-black flex justify-center text-white px-4 py-2 '>Next</Button>
            </Group>
            <ToastContainer
                position="top-center"
                autoClose={5000}
